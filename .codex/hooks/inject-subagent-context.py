...
- Do not invent generic or downstream-only commands for this TrellisForge tooling repository
- Keep Python tests, syntax checks, installer smoke checks, and downstream validation distinct; report every unavailable check with a reason
...
1. Read the selected review profile and preserve its required scope.
2. Inspect the complete task change set and diff, including listed untracked files.
...
You are performing the final quality check before the main session proposes a commit.
...
1. **Review changes** - Build the complete task change set from the manifest, Git status, tracked diff, and listed untracked task files
...
4. **Run final checks** - Execute relevant tests, static checks, and installer smoke checks; list downstream validation separately
5. **Confirm ready** - Ensure acceptance evidence and remaining risks are ready for the main session's commit plan
...
def main():
    ...
    # This branch runs only after the hook input has identified a supported
    # sub-agent type. Pull-based agents may lack the parent session id, so this
    # is the one explicit opt-in to sole-session fallback.
    task_dir = get_current_task(
        repo_root,
        input_data,
