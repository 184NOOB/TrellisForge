...
def _resolve_active_task(root: Path, input_data: dict):
    ...
    # UserPromptSubmit is a main-session event. Without an exact Codex session
    # id it must show no_task instead of borrowing a sole stale session file.
...
def _resolve_skip_keyword(config: dict) -> str:
    """Read `prompt_injection.skip_keyword`; an empty string disables it."""
...
def prompt_has_skip_keyword(prompt: str, keyword: str) -> bool:
    """Match a standalone skip keyword without matching larger identifiers."""
