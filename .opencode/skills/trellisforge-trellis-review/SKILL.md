...
- `changed-scope`: the actual task diff, listed untracked task files, changed
  files, public interfaces, immediate call sites, and directly applicable fast
  checks.
- `affected-scope`: the complete task change set, affected modules, public
  interfaces, direct call sites, one dependency hop, every acceptance criterion,
  and the Specs and tests proven relevant by impact evidence. Do not expand to
  unrelated modules without evidence.
- `full-scope`: the complete task change set, all affected packages or tooling
  layers, applicable Specs, cross-layer contracts, tests,
...
- Use changed-scope: tracked diff, listed untracked task files, changed files,
  public interfaces, immediate call sites, and directly applicable fast checks.
...
- Run only project-defined affected fast tests, static checks, and installer smoke
  checks. Report each category separately as pass/fail/not run/not applicable with
  reasons; never invent commands for unavailable product or hardware targets.
...
- Use affected-scope: inspect the complete task diff, affected modules, public
  interfaces, direct call sites, and one dependency hop. Do not scan the entire
