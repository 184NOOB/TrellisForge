...
| 通用模板要求运行并不存在的 Web `lint/typecheck`，对嵌入式 C 造成无效验证轮次 | 只运行任务或项目定义的静态检查、测试和目标构建；不可执行的检查在最终报告/最终消息中说明，不能向 `plan.py record` 伪造通过结果；计划本身有问题时走 `block`/`revise` | `.trellis/agents/implement.md`、`.trellis/agents/check.md`、`.claude/agents/`、`.codex/agents/`、`.agents/skills/__PROJECT_PREFIX__-trellis-review/` | 分开报告构建、静态检查、单元测试和实体硬件验证，避免虚构命令引发重复尝试；迁移其他语言时替换为真实检查命令。 |
