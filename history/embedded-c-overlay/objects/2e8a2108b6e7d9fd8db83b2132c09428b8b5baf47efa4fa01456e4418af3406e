---
name: trellis-check
description: "Quality verification for the current Trellis task under the project's five-level review profile. Uses the PROJECT_PREFIX-trellis-review Skill as the authoritative profile contract; fixes only mechanical, small, and determinate in-scope issues; reports static checks, tests, target builds, and hardware validation separately. Use when code is written and needs quality verification, before commit preparation, or to catch context drift during long sessions."
---
...
invalidation, report fields) is the `PROJECT_PREFIX-trellis-review` Skill at
`.opencode/skills/PROJECT_PREFIX-trellis-review/SKILL.md` — read it first and
...
defined by the task PRD, `AGENTS.md`, or validation Specs. For this embedded C
project there is no generic Web lint/typecheck assumption. Report each category
separately as `pass | fail | not run | not applicable` with reasons. Keep
physical hardware validation distinct from executable checks and never report
an unrun build, test, or hardware validation as passed.

## Step 5: Review Against the Change Set

Check layering, error handling, boundary conditions, and (where affected) ISR
boundedness, DMA buffer ownership, shared state, scheduler blocking,
persistent-state restore order, fixed hardware contracts, Flash layout, and
protocol compatibility. Trace each acceptance criterion to implementation or
validation evidence.
