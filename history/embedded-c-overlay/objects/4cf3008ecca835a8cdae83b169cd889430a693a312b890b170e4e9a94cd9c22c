...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not gate.ok:
        ...
        for error in gate.errors:
            print(f"  - {error}")
        print()
        print("Complete the evidence/decision convergence pass, present the final")
        print("planning summary, obtain approval in a subsequent user message, then")
        print("persist planning_ready=true and plan_approved=true before retrying.")
        return 1
