...
- **Active task status `planning` + no `prd.md`** → Phase 1.1. Load the
  ...
  `PROJECT_PREFIX-trellis-grill-adapter`.
...
| User intent | Skill |
|---|---|
| New feature / unclear requirements | `trellis-brainstorm` (+ `grill-me` / `PROJECT_PREFIX-trellis-grill-adapter`) |
| About to write code | `trellis-before-dev` |
| Done coding / quality check | `trellis-check` under `PROJECT_PREFIX-trellis-review` |
