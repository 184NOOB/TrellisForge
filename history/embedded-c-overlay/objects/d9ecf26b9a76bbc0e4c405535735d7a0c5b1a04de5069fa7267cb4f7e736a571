...
Classify paths under `.trellis/workspace/` and `.trellis/tasks/` separately. They are managed by `add_session.py` and `task.py archive`, but this project keeps `session_auto_commit: false`, so the scripts leave those bookkeeping changes uncommitted.
...
At minimum: the current active task (if any). Plus any extra tasks the user confirmed in Step 1. Archiving moves task files and updates task metadata; with `session_auto_commit: false`, it does not stage or commit them.
...
Use the work-commit hashes produced in Phase 3.4 (visible in Step 1's `Recent commits` list, or via `git log --oneline`) for `--commit`. This updates the journal only; with `session_auto_commit: false`, it does not stage or commit the journal.

After the script finishes, show the archive/journal diff and propose a separate bookkeeping commit plan. Run `git add` or `git commit` only after fresh, explicit user approval. Do not push.
