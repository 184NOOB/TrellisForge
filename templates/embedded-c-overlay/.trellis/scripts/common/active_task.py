...
def resolve_active_task(
    ...
    *,
    ...
    A stale session task is returned as stale. Main sessions and ordinary CLI
    callers fail closed when no exact context identity is available: existing
    runtime session files are reported as ambiguous instead of being borrowed.

    Pull-based platform sub-agents that are explicitly known to lack the
    parent's session id may opt into single-session inference by passing
    ``allow_single_session_fallback=True``. If exactly one session file exists,
    that opt-in returns its task with source_type="session-fallback". Two or
    more files remain ambiguous.
    """
    ...
    if context_key:
        ...
        # For ordinary callers, an exact new-session identity with no pointer
        # is authoritative. An explicitly marked pull-based sub-agent may keep
        # going to the compatibility fallback because its child id is not the
        # parent session whose task it needs.
        if not allow_single_session_fallback:
    ...
    if _runtime_session_files(repo_root):
        return ActiveTask(None, "ambiguous")

    return ActiveTask(None, "none", context_key)


def _runtime_session_files(repo_root: Path) -> list[Path]:
    """Return runtime session files in stable order."""
...
def _resolve_single_session_fallback(repo_root: Path) -> ActiveTask | None:
    ...
    """
    session_files = _runtime_session_files(repo_root)
